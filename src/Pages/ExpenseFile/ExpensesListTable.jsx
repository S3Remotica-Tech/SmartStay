  ...
  return (
    <>
      ...
      >
        ...
        <td
          className="w-[100px] px-2 py-1 block truncate"
          title={props.item.accountHolderName}
        >
          {props.item.accountHolderName && props.item.accountHolderName} -{" "}
          {props.item.bankName}
        </td>
        ...
        >
            ...
            <div className="w-full mt-3">
              <Select
                ...
                onChange={(selectedOption) =>
                  handleAssetname(selectedOption?.value)
                }
                styles={{
                  control: (base, state) => ({
                    ...base,
              ...
              {state.AssetList.assetList &&
                state.AssetList.assetList.length === 0 && (
                  <label className="pb-1 !text-sm !text-red-500 !font-gilroy !font-medium">
                    Please add an &apos;Asset&apos;option in Asset page,
                    accessible after adding an expense.
                  </label>
                )}
              {assetnameerror && (
          ...
          {formLoading && (
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex items-center justify-center opacity-75 z-10">
              <div className="h-10 w-10 rounded-full border-t-4 border-blue-600 border-r-4 border-r-transparent animate-spin"></div>
            </div>
          )}
        </Modal>
      )}
