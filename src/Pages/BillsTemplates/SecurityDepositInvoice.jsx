...
const SecurityDepositInvoiceTemplate = ({
  ...
  const securityDepositInvoiceTemplate =
    BillsTemplateList &&
    BillsTemplateList.templates?.find(
  ...
  useEffect(() => {
    if (securityDepositInvoiceTemplate) {
      ...
      const templateTheme =
        securityDepositInvoiceTemplate?.invoiceTemplateColor;
    ...
    if (templateThemes && Object.keys(templateThemes).length > 0) {
      if (templateThemes?.logoPreview) {
        setLogoPreview(templateThemes.logoPreview);
      }

      if (templateThemes?.signaturePreview) {
        setSignaturePreview(templateThemes.signaturePreview);
      }

      if (templateThemes?.mobilenum) {
        setMobileNum(templateThemes.mobilenum);
      }

      if (templateThemes?.email) {
        setEmail(templateThemes.email);
      }

      if (templateThemes?.prefix) {
        setPrefix(templateThemes.prefix);
      }

      if (templateThemes?.suffix) {
        setSuffix(templateThemes.suffix);
      }

      if (templateThemes?.qrImage) {
        setQrImage(templateThemes.qrImage);
      }

      if (templateThemes?.terms) {
        setTerms(templateThemes.terms);
      }

      if (templateThemes?.color) {
        setColor(templateThemes.color);
        setUseGradient(false);
      }
    }
              ...
              <div className="row px-4 mt-1">
                ...
                <div className="col-md-6  ps-5 ">
                  <div className="row">
                    ...
                    <div
                      className="col-6 text-muted  text-start mt-1"
                      style={{
                        fontSize: "9px",
                        fontFamily: "Gilroy",
                        fontWeight: 600,
            ...
            <div className="px-4" style={{ marginTop: 10 }}>
              <div className="row">
                <div className="col-md-6 mb-3">
                  ...
                  >
                    Account No :{" "}
                    {securityDepositInvoiceTemplate?.accountNumber || "-"}
                  </p>
                  <p
                    className="mb-1"
                    style={{
                  ...
                  >
                    IFSC Code :{" "}
                    {securityDepositInvoiceTemplate?.ifscCode || "-"}
                  </p>
                  <p
                    className="mb-1"
                    style={{
                ...
                <div
                  className="col-md-4 d-flex flex-column align-items-end "
                  style={{}}
                >
