...
function StaticExample({ show, handleClose , currentItem}) {
    ...
    const handleAddAsset = () => {
        if(assetName && vendorName && brandName && serialNumber &&  productCount && purchaseDate && price ){
            dispatch({ type: 'ADDASSET', payload: { asset_name: assetName, vendor_id: vendorName, brand_name: brandName, serial_number: serialNumber, product_count: productCount, purchase_date: purchaseDate, price: price ,id: id} })
...
setTotalPrice('');
    ...
    useEffect(() => {
        if (currentItem) {
            ...
            setPurchaseDate(moment(currentItem.purchase_date).format('YYYY-MM-DD'));
