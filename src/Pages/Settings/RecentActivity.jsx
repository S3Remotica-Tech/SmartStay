    ...
    return (
      <div className="flex mb-3">
        <div className="flex flex-col items-center mr-3">
          ...
          {!isLast && <div className="w-[2px] flex-1 bg-[#D1D5DB] mt-1"></div>}
        </div>

        <div className="pb-3 w-full">{children}</div>
      </div>
    );
  };
  StepItem.propTypes = {
  ...
  return (
    ...
    <div className="mt-2 w-full max-h-[calc(100vh-220px)] overflow-y-auto overflow-x-hidden">
      {recentActivities.map((item, index) => (
        <StepItem key={index} isLast={index === recentActivities.length - 1}>
          <div className="text-sm md:text-xs lg:text-base">{item.update}</div>

          <div className="text-xs md:text-[11px] lg:text-sm">
            {item.description}
          </div>

          <div className="text-[11px] md:text-[10px] lg:text-xs">
