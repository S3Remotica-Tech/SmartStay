...
function LongStayRecurringModal() {
  ...
  const reminderRange = Array.from({ length: dueDays || 0 }, (_, i) => i + 1);

  const isDisabled =
    !state.UsersList.hotelDetailsinPg?.canModifyBilling ||
    billingMethod === "joining_date_based";

  const isDisabledType = !state.UsersList.hotelDetailsinPg?.canModifyBilling;

  const today = new Date();
  const year = today.getFullYear();
  const month = today.getMonth();

  const dueDateObj = new Date(year, month, billingDate + (dueDays - 1));
  ...
  const handleAddRow = () => {
    setPayments([...payments, { fromDay: "", toDay: "", amountPerDay: "" }]);
  };
  ...
  useEffect(() => {
    if (state?.Settings?.SettingsBillsGetRecurring) {
      ...
      const billingModel =
        apiData?.billingModel?.toLowerCase().trim() === "prepaid"
          ? "prepaid"
          : "postpaid";
  ...
  const handleSaveChanges = () => {
    ...
    const initialData = {
      ...
      billingMethod: initialValues?.billingMethod,
      reminderDays:
        initialValues?.reminderDays?.map((item) => item.value) || [],
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        ...
        !duePickerRef.current.contains(event.target)
      ) {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        gracePickerRef.current &&
        !gracePickerRef.current.contains(event.target)
      ) {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        noticePickerRef.current &&
        !noticePickerRef.current.contains(event.target)
      ) {
    ...
    <>
      ...
      <div className="sticky top-0 left-0 right-0 z-50 bg-white flex flex-col md:flex-row justify-between items-center min-h-[50px] px-1.5 whitespace-nowrap font-gilroy">
        <div>
          ...
          <div className="-ml-1">
            <label className="text-[#7C7C7C] font-medium text-sm">
              Billing Rule /{" "}
            </label>
        ...
        <div className="bg-white rounded-xl shadow-sm p-3 font-gilroy mb-3">
          ...
          <div className="flex gap-4 my-2">
            <div
              ...
              className={`flex items-center gap-3 p-2 max-h-[150px] rounded-lg border w-full transition
  ${isDisabledType ? "bg-gray-100 cursor-not-allowed opacity-60" : "cursor-pointer"}
  ${
    billingMethod === "fixed"
      ? "border-[#4E61F6] bg-[#EEF2FF] ring-2 ring-[#4E61F6]/30"
...
`}
            >
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">
                  Its automatically calculated based on bill start date
                </p>
              </div>
            </div>
            ...
            <div
              onClick={() => {
                if (isDisabledType) return;
                handleChange("joining_date_based");
              }}
            ...
            >
              ...
              <div>
                ...
                <label className="text-xs text-gray-500 cursor-pointer lg:whitespace-nowrap">
                  Invoices are generated based on each tenants join date.
                </label>
              </div>
            </div>
          </div>
  ...
  ${isDisabled ? "bg-gray-100 cursor-not-allowed" : "bg-white cursor-pointer"}`}
              >
                <span
                  className={billingDate ? "text-gray-900" : "text-gray-400"}
                >
                  {billingDate
                    ? billingDate.toString().padStart(2, "0")
                    : "Select Date"}
                </span>
            ...
            <div>
              ...
              <div className="bg-gray-100 border border-gray-200 rounded-md px-3 py-2.5 min-h-[40px] text-sm text-gray-500">
                {billingDate
                  ? `${endDayDate.toString().padStart(2, "0")}`
                  : "—"}
              </div>
              {billingMethod === "fixed" && (
                <p className="text-xs text-gray-400 mt-1">
        ...
        ${
          ...
          {billingMethod === "joining_date_based" && (
            <div className="mt-4 flex items-center gap-2 bg-[#FFF6E6] border border-[#FFF6E6] text-[#795216] text-xs px-3 py-2 rounded-md ">
              <span>
                <AiOutlineExclamationCircle color="#795216" size="16" />
              </span>
              This method doesn’t allow to edit, cause each tenant will have a
              personal billing cycle based on their joining date.
            </div>
          )}
          ...
          {billingMethod === "fixed" && (
            <div className="bg-white rounded-xl shadow-sm p-3 font-gilroy">
              ...
              <label className="text-sm text-[#616161] mt-1 mb-3 font-medium block">
                Tenants joining within the first few days of the billing cycle
                will be charged the full month rent. After this period, rent
                will be calculated based on stay duration.
              </label>
              ...
              <div className="relative max-w-xs" ref={gracePickerRef}>
                ...
                >
                  <span
                    className={gracePeriod ? "text-gray-900" : "text-gray-400"}
                  >
                    {gracePeriod
              ...
              {gracePeriod > 0 && (
                <div className="mt-4 flex items-center gap-2 bg-[#D0DFFF] border border-[#D0DFFF] text-[#1E45E1] text-xs px-3 py-2 rounded-md">
                  ...
                  Full rent will apply if tenant joins from <b>{startDay}</b> to{" "}
                  <b>{endDay}</b> of the month. Prorated rent applies from{" "}
                  <b>{startFrom}</b> onwards.
                </div>
              )}
            </div>
          )}
          ...
          <div className="bg-white rounded-xl shadow-sm p-3 font-gilroy">
            ...
            ${
              ...
              <div className="relative" ref={reminderPickerRef}>
                ...
                <div
                  onClick={() =>
                    dueDays && setOpenReminderPicker(!openReminderPicker)
                  }
                  ...
                  <div className="absolute z-50 mt-2 w-full bg-white border rounded-lg shadow-md p-2">
                    <div className="grid grid-cols-5 gap-3">
                      {reminderRange.map((day) => {
                        const selected = reminderDays?.some(
                          (d) => d.value === day,
                        );
                        ...
                        return (
                          <button
                            ...
                            className={`w-10 h-10 rounded-full text-xs flex items-center justify-center
${selected ? "bg-blue-600 text-white" : "text-gray-700 hover:bg-gray-200"}`}
                          >
                            {day.toString().padStart(2, "0")}
                          </button>
                        );
          ...
          <div className="bg-white rounded-xl shadow-sm p-3 font-gilroy">
            ...
            <div className="grid grid-cols-2 gap-6 items-start">
              <div className="relative">
                ...
                >
                  <span
                    className={noticeDays ? "text-gray-900" : "text-gray-400"}
                  >
                ...
                {openNoticePicker && (
                  <div
                    ref={noticePickerRef}
                    className="absolute z-50 mt-2 w-full bg-white border rounded-lg shadow-md p-2"
                  >
                    <div className="grid grid-cols-5 gap-3">
              ...
              ${
                ...
                {noticeDays && (
                  <div className="mt-3 flex items-center gap-2 bg-[#D0DFFF] border border-[#D0DFFF] text-[#1E45E1] text-xs px-3 py-2 rounded-md whitespace-nowrap w-fit">
                    <AiOutlineExclamationCircle color="#1E45E1" size="16" />
                    Tenants must serve the notice period days before leaving the
                    property
                  </div>
                )}
              </div>
        ...
        lateFeeType === "tiered"
                      ...
                      <div className="grid grid-cols-2 gap-6 mt-4">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-400 mt-2">
                            Late fees will not exceed this amount regardless of
                            delay duration
                          </p>
                        </div>
                      </div>
                        ...
                        <div className="border rounded-lg  mt-3">
                          <table className="w-full">
                            <thead className="bg-gray-100">
                              <tr>
                                <th className="px-4 py-1.5 rounded-tl-lg text-xs text-gray-500 whitespace-nowrap">
                                  FROM DAY
                                </th>
                                <th className="px-4 py-1.5  text-xs text-gray-500 whitespace-nowrap">
                                  TO DAY
                                </th>
                                <th className="px-4 py-1.5  text-xs text-gray-500 whitespace-nowrap">
                                  AMOUNT PER DAY (₹)
                                </th>
                                <th className="px-4 py-1.5 rounded-tr-lg text-xs text-gray-500 whitespace-nowrap">
                                  ACTION
                                </th>
                              </tr>
                            </thead>
                              ...
                              {payments.map((payment, index) => (
                                <tr key={index} className="">
                                  ...
                                  <td className="px-4 py-2 ">
                                    ...
                                    >
                                      Remove
                                      <MinusCirlce size="16" color="#FF0000" />
                                    </button>
                                  </td>
                                </tr>
                              ))}
                        ...
                        <div className="mt-4">
                          ...
                          <div className="relative w-[220px]">
                            ...
                            <label className="text-xs text-gray-500 whitespace-nowrap">
                              Late fees will not exceed this amount regardless
                              of delay duration
                            </label>
                          </div>
                        </div>
          ...
          <div className="flex justify-end gap-3 mt-6">
            ...
            >
              <ArchiveBook
                size="16"
                color={formLoading ? "#9CA3AF" : "#FFFFFF"}
              />{" "}
              Save Changes
            </button>
          </div>
