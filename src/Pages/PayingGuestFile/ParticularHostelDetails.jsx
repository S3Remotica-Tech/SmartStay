...
function ParticularHostelDetails(props) {
  ...
  const [deleteRoomDetails, setDeleteRoomDetails] = useState({
    hostel_Id: null,
    floor_Id: null,
    room_Id: null,
  });
  ...
  useEffect(() => {
    if (
      state.PgList.statusCodeCreateRoom === 201 ||
      state.PgList.statusCodeUpdateRoom === 200
    ) {
      if (props.floorID) {
        dispatch({
          type: "GETALLROOMSLIST",
  ...
  useEffect(() => {
    if (
      ...
      state.PgList.updateBedStatusCode === 201
    ) {
      setTimeout(() => {
  ...
  useEffect(() => {
    if (
      state?.Booking?.statusCodeForAddBooking === 200 ||
      state.UsersList?.statusCodeForAddUser === 201 ||
      state.UsersList?.statusCodeForAddCustomerSaveInfo === 201
    ) {
      dispatch({
