    ...
    return (
        ...
        <>
            <tr key={props.item.id} style={{ color: "#000", fontFamily: "Gilroy", fontSize: "14px", fontStyle: "normal", lineHeight: "normal", alignItems: 'center', marginTop: '10px' }} className='m-2' >
                ...
                <td style={{ fontFamily: 'Gilroy, sans-serif' }}>
                    <div className="d-flex row align-items-center mt-2">
                        <div style={{ display: 'flex' }}>
                            ...
                            <div style={{ fontFamily: 'Gilroy, sans-serif', fontSize: '15px', wordWrap: 'break-word', marginLeft: '8px', color: "#222", fontStyle: 'normal', lineHeight: 'normal', fontWeight: 600 }}>{props.item.Name}</div><br />
                        </div>
                    </div>
                </td>
                <td style={{ cursor: 'pointer', fontFamily: 'Gilroy, sans-serif', fontSize: '14px', marginLeft: '8px', fontStyle: 'normal', lineHeight: 'normal', fontWeight: 600 }} onClick={() => handleInvoicepdf(props.item)} className='pt-3'>#{props.item.Invoices == null || props.item.Invoices == '' ? '0.00' : props.item.Invoices}</td>
                <td style={Tablebodystyle}><span style={{ backgroundColor: '#EBEBEB', padding: '8px 12px', color: '#000000', borderRadius: '14px' }} >{moment(props.item.Date).format('DD MMM YYYY').toUpperCase()}</span></td>
                ...
                <td style={{ fontFamily: 'Gilroy, sans-serif', fontSize: '16px', color: "#000", fontStyle: 'normal', lineHeight: 'normal', fontWeight: 500, paddingTop: '17px' }} > ₹{props.item.Amount.toLocaleString('en-IN')}</td>
                <td style={{ fontFamily: 'Gilroy, sans-serif', fontSize: '16px', color: "#000", fontStyle: 'normal', lineHeight: 'normal', fontWeight: 500, paddingTop: '17px' }} >₹{props.item.BalanceDue.toLocaleString('en-IN')}</td>
                <td style={{
                    paddingTop: '17px',
                    color: props.item.BalanceDue === 0 ? "green" : "red",
                    fontWeight: 500
                }}>
                    {props.item.BalanceDue === 0 ? <span style={{ backgroundColor: '#D9FFD9', padding: '8px 12px', color: '#000000', borderRadius: '14px', fontFamily: 'Gilroy, sans-serif' }}>Paid</span> : <span
                        // onClick={() => handleShow(props.item)}
                        style={{ cursor: 'pointer', backgroundColor: '#FFD9D9', fontFamily: 'Gilroy, sans-serif', padding: '8px 12px', color: '#000000', borderRadius: '14px' }}>Unpaid</span>}</td>
