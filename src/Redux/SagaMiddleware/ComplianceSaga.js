...
function* handleParticularVendorOverview(action) {
  try {
    ...
    if (response?.status === 200) {
      yield put({
        type: "PARTICULAR_VENDOR_OVERVIEW",
        payload: {
          response: response.data || [],
          statusCode: response?.status,
        },
  ...
  try {
    ...
    if (response?.status === 200) {
      yield put({
        ...
        payload: {
          response: response.data.complaintsList || [],
...
function* handleVendorCustomizeData(action) {
  try {
    const response = yield call(vendorCustomizeData, action.payload);
    var toastStyle = {
      backgroundColor: "#E6F6E6",
      color: "black",
      width: "100%",
...
function* ComplianceSaga() {
  ...
  yield takeEvery("CUSTOMIZE_VENDOR_SAGA", handleVendorCustomizeData);
