...
function* handleSelfTranferInitialize(action) {
  try {
    const response = yield call(selfTranferInitialize, action.payload);
    const hostelId = GlobalHostelId(response);
    if (hostelId) {
      yield put({ type: "SAVE_RESPONSE_HOSTEL", payload: hostelId });
    }
    ...
    if (response?.status === 200) {
      yield put({
        type: "SELF_TRANSFER_INITIALIZE_REDUCER",
        payload: {
          response: response.data,
          statusCode: response?.status,
        },
