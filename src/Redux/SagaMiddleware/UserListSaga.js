...
import {
  ...
  tenantCustomizeData,
  finalAddRoomReading,
  TenantUploadDocument,
  deleteTenantUploadDocument,
  deleteTemplatesImages,
  cancelCheckoutInitialize,
  getInitializeCheckout,
  EditTenantAmount,
  editAdvanceAmount,
  deleteReading,
  editBasicDetails,
  CancelCheckOutCustomer,
  getParticularCustomerReading,
  getParticularRoomReading,
  getCustomerReading,
  cancelBookingGet,
  bookingToCheckIn,
  KYCReminder,
  ...
  deleteCustomer,
  AvailableCheckOutCustomer,
  DeleteCheckOutCustomer,
  AddCheckOutCustomer,
  getCheckOutCustomer,
  AddWalkInCustomer,
...
function* handleDeleteGlobalTemplatesImages(args) {
  try {
    ...
    if (response?.status === 204) {
      yield put({
        type: "DELETE_GLOBAL_TEMPLATES_IMAGES_REDUCER",
        payload: { response: response.data, statusCode: response?.status },
      });
...
function* handleTenantUploadDocument(datum) {
  try {
    ...
    if (error.status) {
      yield put({
        type: "TENANT_DOCUMENT_UPLOAD_ERROR",
        payload: error.response.data,
      });
      toast.error(`${error.response.data}`, {
        position: "bottom-center",
...
function* handleCancelCheckoutInitialize(reading) {
  try {
    ...
    if (response?.status === 200) {
      yield put({
        type: "INITIALIZE_CANCEL_CHECKOUT",
        payload: { response: response.data, statusCode: response?.status },
      });
    }
  } catch (err) {
...
function* handleEditBasicDetails(reading) {
  try {
    ...
    if (error.code === "ERR_BAD_REQUEST") {
      if (error.status === 400) {
        yield put({
          type: "ALREADY_MOBILE_BASIC_ERROR",
          payload: error.response.data,
        });
      }
    }
...
function* handleGetParticularCustomerReading(reading) {
  try {
    ...
    if (response?.status === 200) {
      yield put({
        type: "GET_PARTICULAR_CUSTOMER_READING",
        payload: { response: response.data, statusCode: response?.status },
      });
    }
...
function* handleGetParticularRoomReading(reading) {
  try {
    ...
    if (response?.status === 200) {
      yield put({
        type: "GET_PARTICULAR_ROOM_READING",
        payload: { response: response.data, statusCode: response?.status },
      });
    }
...
function* handleCancelBookingGet(reading) {
  try {
    ...
    if (response?.status === 200) {
      yield put({
        type: "INITIALIZE_CANCEL_BOOKING",
        payload: { response: response.data, statusCode: response?.status },
      });
    } else {
...
function* handleFinalSettlementAddRoomReading(reading) {
  try {
    ...
    if (response?.status === 201 || response?.status === 200) {
      yield put({
        type: "FINAL_SETTLEMENT_ADD_ROOM_READING",
        payload: { response: response.data, statusCode: response?.status },
      });
  ...
  try {
    ...
    if (response?.status === 200) {
      ...
      yield put({
        type: "NO_USER_LIST",
        payload: {
          response: response.data.hostelData || [],
  ...
  try {
    ...
    if (response?.status === 200) {
      yield put({
        ...
        payload: response.data.hostel_data,
        statusCode: response?.status,
      });
    } else {
      yield put({ type: "ERROR", payload: response?.data?.message });
  ...
  try {
    ...
    if (response?.status === 200) {
      yield put({
        type: "KYC_VALIDATE",
        payload: {
          response: response.data.result.ref_id,
          statusCode: response?.status,
        },
      });
  ...
  try {
    ...
    if (statusCode === 200) {
      yield put({
        ...
        payload: { response: response.data || response, statusCode },
      });
  ...
  try {
    ...
    if (response?.status === 200) {
      yield put({
        ...
        payload: { response: response.data.user_list },
      });
    }
...
function* handleKYCVerifyNew(action) {
  try {
    ...
    if (response?.status === 200) {
      ...
      toast.success(
        `${response?.data?.result?.message || "KYC verified successfully"}`,
        {
          position: "bottom-center",
          autoClose: 2000,
          hideProgressBar: true,
      ...
      yield put({
        type: "ERROR",
        payload: response?.data?.message || "KYC verification failed",
      });
    }
...
function* handleConfirmCheckoutDueCustomer(data) {
  try {
    ...
    if (response?.status === 200) {
      yield put({
        type: "CONFIRM_CHECKOUT_DUE_CUSTOMER",
        payload: { statusCode: response?.status },
      });
  ...
  try {
    ...
    if (response?.status === 200) {
      yield put({
        ...
        payload: {
          response: response.data.listCustomers,
          statusCode: response?.status,
        },
      });
    } else {
...
function* handleTenantCustomizeData(action) {
  try {
    ...
    if (response?.status === 200) {
      yield put({
        type: "CUSTOMIZE_TENANT_COLUMNS_REDUCER",
        payload: { response: response.data, statusCode: response?.status },
      });
...
function* UserListSaga() {
  ...
  yield takeEvery(
    "FINALSETTLEMENTADDROOMREADINGSAGA",
    handleFinalSettlementAddRoomReading,
  );
  ...
  yield takeEvery(
    "GETPARTICULARCUSTOMERREADING",
    handleGetParticularCustomerReading,
  );
  ...
  yield takeEvery(
    "GETCONFIRMCHECKOUTCUSTOMER",
    handlegetConfirmCheckOUtCustomer,
  );
  ...
  yield takeEvery(
    "CONFIRMCHECKOUTDUECUSTOMER",
    handleConfirmCheckoutDueCustomer,
  );
