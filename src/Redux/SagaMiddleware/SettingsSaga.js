...
function* handleCategoryAdd (params) {
   ...
   if (response.status === 200){
      yield put ({type : 'EXPENCES_ADD' , payload:{response:response.data, statusCode:response.status , message: response.data.message}})
   }
   else {
      yield put ({type:'ERROR', payload:response.data.message})
   }
...
function* handleComplainttypelist (action){
    ...
    if (response.status === 200){
       yield put ({type : 'COMPLAINT_TYPE_LIST' , payload:{response:response.data, statusCode:response.status , message: response.data.message}})
    }else if(response.status === 401){
      Swal.fire({
         icon: 'warning',
         title: 'Error',
...
function* handleDeleteComplainttype(action) {
 ...
 console.log(" response", response)
   if (response.status === 200){
      yield put ({type : 'DELETE_COMPLAINT_TYPE' , payload:{response:response.data, statusCode:response.status}})
   }
   else {
      yield put ({type:'ERROR', payload:response.data.message})
   }
...
function* handleEBBillingUnitAdd (params) {
  ...
  if (response.status === 200){
     yield put ({type : 'EB_BILLING_UNIT_ADD' , payload:{response:response.data, statusCode:response.status , message: response.data.message}})
  }
  else {
     yield put ({type:'ERROR', payload:response.data.message})
  }
