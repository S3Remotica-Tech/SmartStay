...
import {
  ...
  VendorCategoryList,
  PlanList,
  ChangeRoomHostelElectricity,
  getModules,
  RecurringRole,
  AddExpencesCategory,
  EditExpencesCategory,
  ExpencesCategorylist,
  DeleteExpencesCategoryList,
  Addcomplainttype,
  DeleteVendorCategoryList,
  Complainttypelist,
  DeletecomplaintType,
  AddEBBillingUnit,
  GetEBBillingUnit,
  ...
  SubscriptionPdfDownload,
  SettingsAddRecurring,
  GetBillsFrequncyTypes,
  GetBillsNotificationTypes,
  SettingsGetRecurring,
  AddInvoiceSettings,
  SettingsGetInvoice,
  AddBillTemplate,
  getTemplateList,
  AddGlobalSettingTemplate,
  SettingsGetGlobal,
  EditGeneral,
  EditStaffUser,
...
function* handleVendorCategorylist(action) {
  try {
    ...
    if (response?.status === 200) {
      yield put({
        type: "VENDOR_CATEGORY_LIST_REDUCER",
        payload: {
          response: response.data,
          statusCode: response?.status,
          message: response?.data?.message,
...
function* handleCreateVendorCategory(params) {
  try {
    const response = yield call(AddVendorCategory, params.payload);

    if (response?.status === 201) {
      yield put({
        type: "VENDOR_CATEGORY_REDUCER",
        payload: {
          response: response.data,
          statusCode: response?.status,
        },
...
function* handleEditSubCategory(params) {
  try {
    ...
    if (response?.status === 200) {
      yield put({
        type: "EDIT-EXPENCES-SUB-CATEGORY",
        payload: {
          response: response.data,
          statusCode: response?.status,
          message: response?.data,
...
function* handleDeleteComplainttype(action) {
  try {
    ...
    if (error.code === "ERR_BAD_REQUEST") {
      ...
      if (ComplaintError) {
        yield put({
          type: "ALREADY_ASSIGNCOMPLAINTTYPE_ERROR",
          payload: ComplaintErrormessage,
        });
      }
    }
  ...
  try {
    ...
    if (response?.status === 201) {
      yield put({
        ...
        payload: { response: response, statusCode: response?.status },
      });
      toast.success(`${response.data}`, {
        position: "bottom-center",
        autoClose: 2000,
  ...
  try {
    ...
    if (response?.status === 200) {
      yield put({
        ...
        payload: {
          response: response.data.pdf_url,
          statusCode: response?.status,
        },
      });
    } else {
